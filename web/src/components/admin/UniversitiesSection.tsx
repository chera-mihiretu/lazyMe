...
import { Building, Edit, Trash2, AlertTriangle, Loader2, Info, ChevronRight } from 'lucide-react';
...
const UniversitiesSection: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setDeleteError("Error deleting university" + err);
    } finally {
      ...
      >
        ...
        {loading ? (
          ...
          >
            ...
            <span className="text-gray-500">No universities found</span>
          </motion.div>
        ) : (
          <motion.div
            className="space-y-3"
          ...
          >
            {universities.slice(0, 4).map((university, index) => (
              <motion.div
                key={university.id}
                className="group bg-gray-50 rounded-xl p-4 border border-gray-100 hover:border-purple-200 hover:bg-purple-50/30 transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    {university.description && (
                      <div className="flex items-center mt-2 text-sm text-gray-500">
                        ...
                        <span className="line-clamp-1">{university.description}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => { 
                        setShowDeleteDialog(university.id); 
                        setDeleteInput(""); 
                        setDeleteError(""); 
                      }}
                    >
        ...
        {/* More Button */}
        <motion.button
          className="w-full mt-6 px-4 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center group"
            ...
            >
              ...
              <div className="mb-4">
                <p className="text-gray-700 mb-3">
                  Type <span className="font-semibold text-red-600">"delete university"</span> to confirm deletion.
                </p>
                <input
                  type="text"
                  value={deleteInput}
