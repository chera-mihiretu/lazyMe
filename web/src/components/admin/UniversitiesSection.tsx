...
const UniversitiesSection: React.FC = () => {
  ...
  return (
    <section className="bg-white rounded-[14px] shadow-[0_2px_8px_#4320d120] px-6 pt-6 pb-4 flex flex-col min-h-[180px]">
      <h3 className="font-bold text-[20px] text-[#4320d1] mb-3">Universities</h3>
      {loading ? (
        <div className="text-[#888] font-medium py-3">Loading...</div>
      ) : error ? (
        <div className="text-[#e53e3e] font-medium py-3">{error}</div>
      ) : universities.length === 0 ? (
        <div className="text-[#888] font-medium py-3">No universities found.</div>
      ) : (
        <div className="flex flex-col gap-2 flex-1">
          {universities.slice(0, 4).map(u => (
            <div key={u.id} className="flex flex-col bg-[#f7f7fa] rounded-lg px-3 py-2 mb-0.5">
              <div className="flex items-center justify-between">
                <span className="text-[#333] font-medium">{u.name}</span>
      ...
      {showDeleteDialog && (
        <div className="fixed top-0 left-0 w-screen h-screen bg-black/20 flex items-center justify-center z-[1000]">
          <div className="bg-white rounded-2xl shadow-[0_4px_32px_#4320d120] px-8 py-6 min-w-[320px] w-full max-w-md flex flex-col gap-4">
            <h3 className="font-bold text-[20px] text-[#e53e3e] mb-2">Delete University</h3>
            <div className="text-[#333] font-medium mb-2">Type <span className="text-[#e53e3e] font-bold">&quot;delete university&quot;</span> to confirm deletion.</div>
            <input
              type="text"
              value={deleteInput}
