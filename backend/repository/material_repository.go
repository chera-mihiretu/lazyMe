...
func (r *materialsRepository) CreateMaterials(ctx context.Context, materials models.Materials) (models.Materials, error) {
	...
	err := r.department.FindOne(ctx, bson.M{"_id": materials.DepartmentID}).Decode(&depa)

	if err != nil {
		if err == mongo.ErrNoDocuments {
			return models.Materials{}, errors.New("department not found")
		}
		return models.Materials{}, err
	}

	fmt.Println("Department found:", depa, " Got:", materials)

	if materials.Year > depa.Years || materials.Year < 0 {
